import { handle, readJson, json, err, nowIso, requireAdmin, getState, computeBoard, publicBoard, HANDLE_RE, randomToken, hashSessionToken } from '../../../../ctf-lib/server.js';
...
export const onRequestPost = handle(async ({ request, env }) => {
	...
	switch (body.action) {
		...
		case 'reopen':
			...
			break;
		case 'freeze': {
			const snap = publicBoard(await computeBoard(env));
		...
		case 'hide':
		case 'unhide':
			await db.prepare('UPDATE participants SET is_hidden = ? WHERE id = ?').bind(body.action === 'hide' ? 1 : 0, Number(body.id)).run();
			break;
		case 'rename': {
			const h = String(body.handle || '').trim();
			if (!HANDLE_RE.test(h)) return err('Handle must be 3–24 characters: letters, numbers, _ or -.');
			try {
				await db.prepare('UPDATE participants SET handle = ?, handle_normalized = ? WHERE id = ?').bind(h, h.toLowerCase(), Number(body.id)).run();
			} catch (e) {
	...
	return json({ ok: true, state: await getState(env).then((s) => ({ ...s, frozen_snapshot: undefined })) });
});
